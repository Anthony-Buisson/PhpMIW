...
if (isset($_GET['id_article']))//bug trouve
    $id = $_GET['id_article'];//bug trouve
else {
...
if (isset($_GET['id_article'])) { ?>
    ...
    <div><i>Publié le <?php echo $newDate ?> par <?php echo $article['name'] ?></i></div>
    <div>
    ...
    <div>
        <?php
        if (count($commentaires)) {//bug trouve
            foreach ($commentaires as $commentaire) {//bug trouve
                ?>
                <div class="commentaire">
                    <?php foreach ($users as $user) {
                        if($commentaire['id_user'] === $user['id']){?>
                        <b><?php echo $user['name'].' : ' ?></b>
                    <?php }} ?>
                    <?php echo nl2br($commentaire['contenu']) //bug trouve?>
                </div>
                <?php
            }
        } else {
    ...
    <div>
        <form method="post" action="saveComment.php"><!--bug trouve-->

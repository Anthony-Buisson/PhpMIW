...
if(!is_writable('upload/pdf/') || !is_writable('upload/image/') || !is_writable('upload/pdf/')){
...
if(isset($_FILES['photo'])){
    ...
    if(($_FILES['photo']['error'] == UPLOAD_ERR_OK) && in_array($_FILES['photo']['type'], ['image/png', 'image/jpg', 'image/jpeg', 'application/pdf', 'image/gif'])){
        ...
        $fichier = time().'-'.str_replace([' ',':'],'_',$_POST['nom']).'.'.$matches[0][count($matches[0])-1];
        if(move_uploaded_file($_FILES['photo']['tmp_name'], $dossier.$fichier)){
            ...
            crop($dossier.$fichier, 150, $dossier);
        }else{
